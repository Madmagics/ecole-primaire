...
func _check_npc_wiring() -> void:
	...
	for npc in npcs_root.get_children():
		...
		## Ajoutee le 2026-09-06 : cette connexion EXACTEMENT dans le sens inverse (QuestionPanel
		## -> giver.resolve_pack, pas giver -> QuestionPanel comme les 4 ci-dessus) manquait dans
		## school.tscn depuis toujours et n'etait pas verifiee ici - decouverte via le badge de
		## limite quotidienne (DailyLimitBadge, voir feedback_parental_control_checkbox.md en
		## memoire projet) qui restait bloque a 0 apres un questionnaire termine. Sans elle,
		## resolve_pack() n'etait JAMAIS appelee : EventBus.pack_completed n'etait jamais emis,
		## donc ni les pieces (Economy.add_coins), ni les statistiques (StatsTracker), ni le
		## compteur quotidien (SaveManager) n'etaient mis a jour - uniquement le texte "tu gagnes
		## X" affiche par QuestionPanel lui-meme restait correct, ce qui rendait le bug invisible
		## a l'oeil sans verifier le solde reel.
		_require(_question_panel.pack_completed, giver.resolve_pack, npc.name, "pack_completed -> resolve_pack (voir [connection] dans school.tscn)")
